...
import { Users, Briefcase, Trophy, Star, UserCheck } from 'lucide-react';
...
export default function AboutStats() {
  ...
  return (
    <div className="px-6 lg:px-12 max-w-[1400px] mx-auto relative z-10 w-full">
      <div className="bg-[#111317]/80 backdrop-blur-xl border border-gray-800 rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.5)] p-6 md:p-8 overflow-hidden">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 md:gap-8 divide-x-0 lg:divide-x divide-gray-800">
          {stats.map((stat, i) => (
            <div key={i} className={`flex flex-col items-center justify-center text-center ${i > 0 ? 'lg:pl-8' : ''}`}>
              <div className="w-14 h-14 rounded-full bg-brand-yellow/10 border border-brand-yellow/20 flex items-center justify-center mb-4 shadow-[inset_0_0_15px_rgba(248,180,23,0.1)] transition-transform hover:scale-110">
                {stat.icon}
              </div>
